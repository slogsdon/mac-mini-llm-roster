#!/usr/bin/env python3
"""Speed-test the full local model roster across role-appropriate prompts.

Runs every prompt against every model so we can verify role assignments are
optimal — both for raw throughput and for output quality.

Hits Ollama directly at :11434 (NOT through LiteLLM) so we can capture
eval_count / eval_duration for accurate tokens-per-second metrics, plus
the separated `thinking` channel for reasoning models. Production traffic
still goes through LiteLLM; this is a diagnostic.

Sequential per model so each model loads at most once — the cold-load tax
falls on the first prompt per model. Subsequent prompts are warm.

Writes JSON results to /tmp/roster_speed_results.json and prints a
markdown-formatted summary to stdout suitable for appending to the vault.
"""
...
# think_override: True = force think:true, False = force think:false,
#                 None = pass nothing (let Ollama decide by capability).
# num_ctx = the isolated-mode default from models.yaml.
# num_predict = Pi's maxTokens from models.json — the budgets the user
# actually deploys with.
MODELS = [
    ("code-autocomplete", "qwen2.5-coder:3b-base",   None,   32_768,  8_192),
    ("fast-general",      "gemma4:e4b-mlx",          False,  65_536, 16_384),
    ("code-review",       "granite4.1:8b",           None,   65_536, 16_384),
    ("general",           "qwen3.5:9b-mlx",          False, 131_072, 32_768),
    ...
    ("capable-large",     "gpt-oss:20b",             True,   65_536, 16_384),
    ("flash",             "glm-4.7-flash:q4_K_M",    True,  131_072, 32_768),
    ("vision",            "qwen3-vl:30b",            True,   65_536, 16_384),
    # 2026-05-23 additions:
    ("general-next",      "qwen3.6:35b-mlx",         True,   65_536, 16_384),
    ("instruct",          "ministral-3:14b",         None,   40_960, 16_384),
    ("efficient",         "lfm2:24b",                None,   32_768, 16_384),
    ("tiny-reason",       "lfm2.5-thinking:1.2b",    True,   32_768,  8_192),
]
...
RESULTS_PATH = "/tmp/roster_speed_results.json"
